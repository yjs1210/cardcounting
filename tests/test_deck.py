...
def test_card_count():
    deck = Deck()
    deck.set_cards([Cards.TEN, Cards.QUEEN, Cards.KING, Cards.TEN, Cards.KING, Cards.ACE, Cards.TEN, Cards.QUEEN , Cards.QUEEN, Cards.JACK, Cards.TEN])
    for i in range(11):
        deck.deal()
    assert deck.get_count() == -11
    
    cards = [Cards.FOUR, Cards.FIVE, Cards.SIX, Cards.SEVEN, Cards.EIGHT, Cards.TEN, Cards.ACE]
    deck = Deck()
...
def test_true_count_and_wager():
    ...
    deck = Deck()
    deck.set_cards([Cards.FOUR, Cards.TWO, Cards.SEVEN, Cards.TEN, Cards.NINE, Cards.TWO])
    for i in range(5):
        deck.deal()
    ...
    deck = Deck()
    deck.set_cards([Cards.FOUR, Cards.TWO, Cards.SEVEN, Cards.TEN, Cards.NINE, Cards.TWO]*100)
    for i in range(6*50):
        deck.deal()
    ...
    deck = Deck()
    deck.set_cards([Cards.TWO, Cards.THREE, Cards.FOUR, Cards.FIVE, Cards.SIX, Cards.SEVEN, Cards.EIGHT, Cards.NINE, Cards.TEN, Cards.JACK, Cards.QUEEN, Cards.KING, Cards.ACE]*100)
    ...
    player = Player(bankroll=100, hard_policy=hard_policy, soft_policy=soft_policy, split_policy=split_policy, betting_policy=betting_policy_alt)
