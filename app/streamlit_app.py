...
def show_cached():
    ...
    scenarios = st.sidebar.selectbox('Choose a Cached Scenario', ['Vegas Rules Aggressive Betting', 'Vegas Rules Conservative Betting'])
    if scenarios == "Vegas Rules Aggressive Betting":
        st.markdown('Below is a result of 5000 session samples of 1000 hands under standard Las Vegas Rules(hit soft 17 and late surrender allowed). Bet sizes employed are $25 for counts of 0 and negatives, and multipliers of 8 for count of 1, 16 for count of 2 and 32 for count of 3+')
    ...
    if scenarios == "Vegas Rules Conservative Betting":
        st.markdown('Below is a result of 5000 session samples of 1000 hands under standard Las Vegas Rules(hit soft 17 and late surrender allowed). Bet sizes employed are $25 for counts of 0 and negatives, and multipliers of 4 for count of 1, 8 for count of 2 and 16 for count of 3+')
...
def run_sims():
    ...
    num_samples = st.sidebar.number_input('Number of Sessions: ', min_value=1, max_value=1000, value=100, step=1)
    iterations = st.sidebar.number_input('Number of Hands per Session: ', min_value=1, max_value=10000, value=1000, step=1)
    num_decks = st.sidebar.number_input('Number of decks in a shoe: ', min_value=1, max_value=8, value=6, step=1)
    cut_card_threshhold = st.sidebar.number_input('Dealer Cut Card Proportion', min_value=.01, max_value=.5, value=.35, step=.01)

    
    bet_multipler_neg = st.sidebar.slider("Betting multiplier for count of 0 and negatives", 0, 100, 1)
    bet_multiplier_1 = st.sidebar.slider("Betting multiplier for count of 1", 1, 100, 8)
    bet_multiplier_2 = st.sidebar.slider("Betting multiplier for count of 2", 1, 100, 16)
    bet_multiplier_3 = st.sidebar.slider("Betting multiplier for count of 3", 1, 100, 32)
    bet_multiplier_4 = st.sidebar.slider("Betting multiplier for count of 4", 1, 100, 32)
    bet_multiplier_5 = st.sidebar.slider("Betting multiplier for count of 5+", 1, 100, 32)

    if button_start:
        wager_amts = [bet_multipler_neg, bet_multipler_neg, bet_multipler_neg,bet_multiplier_1,bet_multiplier_2,bet_multiplier_3,bet_multiplier_4,bet_multiplier_5]
        ...
        try:
            ...
            for idx, n_samples_small in enumerate(chunks):
                ...
                results = parallel_processing(player, num_decks = num_decks, iterations= iterations, n_samples = n_samples_small, threshold=cut_card_threshhold)
...
def layout_results(final_results):
    ...
    mean, conf_low, conf_high = get_conf_interval(final_results, .9)
    ...
    st.markdown('90% Confidence Interval: [{}, {}]'.format(round(conf_low,3), round(conf_high, 3)))
...
def run_in_chunks(num: int, divisor: int):
    ...
    return [divisor]*quotient + [remainder] if remainder != 0 else [divisor]*quotient
...
def draw_plotly_boxplot(results):
    fig = go.Figure(data=[go.Box(y=results,
            boxpoints='all', # can also be outliers, or suspectedoutliers, or False
            jitter=0.3, # add some jitter for a better separation between points
            pointpos=-1.8 # relative position of points wrt box
             )])
    fig.update_layout(title_text="Profits($)", xaxis_title = "Distribution", title_x = .5)
...
def get_conf_interval(final_results, confidence=.95):
